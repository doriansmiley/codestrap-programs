# Curriculum

**Curriculum version**: `0.0.1` (see [CHANGELOG](../CHANGELOG.md))

- [Prerequisites](#prerequisites)
- [Intro to Shell Scripting](#intro-to-shell-scripting)
  - [Intro to the Command Line](#intro-to-the-command-line)
  - [Intro to Bash Scripting](#intro-to-bash-scripting)
  - [Mastering Linux Text Editors](#mastering-linux-text-editors)
  - [Intro to Regex](#intro-to-regex)
  - [Intro to Testing](#intro-to-testing)
- [Intro to Linux](#intro-to-linux)
    - [Certification Training and Exam](#)
- [Intro to git](#intro-to-git)
- [Intro to IDEs](#intro-to-ides)
    - [VS Code](#)
    - [WebStorm](#)
- [Intro to Semi Structured Data](#intro-to-semi-structured-data)
    - [YAML Essentials](#yaml-essentials)
    - [Learn HTML](#learn-html)
    - [Intro to XML](#)
    - [Intermediate XML](#)
    - [Into to JSON](#)
    - [Intermediate JSON](#)
    - [Markdown Mastery](#markdown-mastery)
- [Intro to JavaScript](#intro-to-javascript)
    - [Setting up your Environment](#)
        -[Linters](#)
        -[Prettier](#)
        -[Bundlers](#)
        -[Package Managers](#)
        -[Pre-commit Checks](#)
   - [Language Basics](#)
   - [Understanding the Event Loop](#)
   - [Async Programming Basics](#)
- [Intermediate Javascript](#)
    - [Data Structures](#)
        [Lists](#)
        [Maps](#)
        [Arrays](#)
        [Trees](#)
    - [DOM APIs](#)
    - [Fetch and XHR](#)
    - [ES Modules and Classes](#)
    - [Object Oriented Programming in Javascript](#)
    - [Intro to Functional Programming](#)
    - [Functional Programmin in JavaScript](#)
    - [Unit Testing](#)
    - [Certification Training and Exam](#)
- [Intro to Type Systems](#)
    - [Intro to TypeScript](#)
- [UI Layout and Design](#)
    - [Intro to CSS](#)
    - [Intro to SCSS](#)
    - [Material Design](#)
- [Application Frameworks](#)
    - [React](#)
    - [Angular](#)
    - [View](#)
    - [Ionic](#)
- [Intro to NodeJS](#intro-to-node-js)
- [Intro to Docker](#)
    - [Certification Training and Exam](#)
- [Intro to AWS](#intro-to-aws)
    - [Overview](#)
    - [Identity and Access Managment](#)
    - [AWS Networking Basics](#)
    - [Intro to the AWS CLI](#)
    - [Intro to CloudFormation](#)
    - [EC2](#)
    - [S3](#)
    - [Hosting a Static Website](#)
    - [Building Serverless Application on AWS](#)
    - [Running Container Based Workloads on AWS](#)
    - [Certification Training and Exam](#)
- [Agile Software Development](#)
    - [Agile Fundamentals](#)
    - [Jira](#)
    - [Confluence](#)
    - [Certification Training and Exam](#)
- [Final project](#final-project)
    - [Contributing to an Open Source Project](#)

## Prerequisites

- [Intro to Shell Scripting](#intro-to-shell-scripting) assumes you have access to linux based terminal on any of the following:
  - Mac or Linux computer
  - Linux virtual machine or remote workstation
  - Virtual IDE like AWS Cloud 9 or similar
  - If you only have access to Windows, tablet, or Chromebook click here.
- You have an aptitude for math and have completed [pre-algebra](https://www.khanacademy.org/math/pre-algebra#pre-algebra-subject-challenge).

## Intro to Shell Scripting
**Topics covered**:
`command line`
`basics or programming`
`linux text editors`
`basics of linux`

### Intro to the Command Line

Learn how to communicate with computers using code.

**Topics covered**:
`basic commands`
`file systems`
`basic envrionment setup`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[Learn the Command Line](https://www.codecademy.com/learn/learn-the-command-line) | 8 hours | 8 hours/week | none | [chat](https://discord.gg/syA242Z)
[YouTube Videos](https://www.youtube.com/playlist?list=PLvp1Riqm5kiO3ZTqqD6rwuwws6ulbiWoW) | 3 hours | na | na | na

### Mastering Linux Text Editors

Master the most fundamental tool for every software engineer: the text editor.

**Topics covered**:
`Linux text editors vim and vi`
`edit, cut, copy paste,etc`
`find and replace`
`running commands`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[Text Editing with Vim Vi iMproved](https://acloud.guru/overview/be02e4b7-edf6-49b8-970d-7b322bbd862b) | 7.5 hours | 8 hours/week | [Intro to Bash Scripting](#intro-to-bash-scripting) | [chat](https://discord.gg/syA242Z)

### Intro to Bash Scripting

Learn the basics of programming in bash.

**Topics covered**:
`control flow`
`inputs and outputs`
`functions`
`debugging`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[The System Administrator's Guide to Bash Scripting](https://acloud.guru/overview/bccc6769-38e7-4a7f-8255-6914b7244caf) | 7.5 hours | 8 hours/week | [Intro to the Command Line](#intro-to-the-command-line) | [chat](https://discord.gg/syA242Z)
[YouTube Videos](https://www.youtube.com/playlist?list=PLvp1Riqm5kiO3ZTqqD6rwuwws6ulbiWoW) | 3 hours | na | na | na

## Intro to Linux

## Intro to git

## Intro to Semi Structured Data
**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

### YAML Essentials

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[YAML Essentials](https://acloud.guru/overview/90dd551f-91a6-4b91-a0a8-d4905521f641) | 7 hours | 8 hours/week | [Intro to Bash Scripting](#intro-to-bash-scripting) | [chat](https://discord.gg/syA242Z)

### Learn HTML

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[Learn HTML](https://www.codecademy.com/learn/learn-html) | 9 hours | 8 hours/week | na | na

### Learn XML Crash Course

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[Learn XML Crash Course](https://www.udemy.com/course/learn-xml-crash-course/) | 2 hours | 8 hours/week | na | na

### XML and XML Schema Definition in Easy Steps

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[XML and XML Schema Definition in Easy Steps](https://www.udemy.com/course/xml-and-xml-schema-definition-in-easy-steps/) | 2 hours | 8 hours/week | na | na

### Introduction to JavaScript Object Notation

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[JSON in Action](Introduction to JavaScript Object Notation) | 1.5 hours | 2 hours/week | na | na

### JSON in Action

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[JSON in Action](https://www.udemy.com/course/json-in-action/) | 4.5 hours | 5 hours/week | na | na

### Markdown Mastery

TODO.

**Topics covered**:
`Basics of semi-structured data`
`yaml`
`html`
`xml`
`json`

Courses | Duration | Effort | Prerequisites | Discussion
:-- | :--: | :--: | :--: | :--:
[Markdown Mastery](https://www.udemy.com/course/markdown/) | 4.5 hours | 5 hours/week | na | na

## Intro to Javascript
